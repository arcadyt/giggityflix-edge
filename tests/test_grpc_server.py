...
class TestGrpcServer:
    ...
    async def test_message_cancelled_error(self, mock_stream_manager, mock_message_handler, 
        ...
        # Create a request iterator that raises CancelledError
        async def mock_aiter():
            yield mock_peer_message
            raise asyncio.CancelledError()
            
        request_iterator = MagicMock()
        request_iterator.__aiter__.return_value = mock_aiter()
    ...
    async def test_start_grpc_server_no_tls(self, mock_stream_manager, mock_message_handler):
        ...
        with patch('src.grpc_server.grpc_aio_server', return_value=server_mock), \
            ...
            # Verify log message
            mock_logger.info.assert_called()
            assert "insecure" in mock_logger.info.call_args[0][0].lower()
    ...
    async def test_start_grpc_server_with_tls(self, mock_stream_manager, mock_message_handler):
        ...
        with patch('src.grpc_server.grpc_aio_server', return_value=server_mock), \
             ...
             patch('src.grpc_server.open', create=True) as mock_open, \
            ...
            # Verify log message
            mock_logger.info.assert_called()
            assert "tls" in mock_logger.info.call_args[0][0].lower()
